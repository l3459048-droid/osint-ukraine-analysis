# OSINT Ukraine Analysis — local-first v0.6
...
└─────────────────────────────────────┘
      ↓
пассивное обслуживание библиотеки
scan → index → 1 перевод за цикл
```
...
На Windows после установки можно использовать `start_local.bat`.
...
Пока `osint-local serve` запущен, v0.6 может обслуживать библиотеку автоматически. По умолчанию примерно раз в минуту система:

1. проверяет новые/изменённые файлы;
2. обновляет semantic index, если Sentence Transformers установлен;
3. переводит **не более одного** ещё не переведённого English/Ukrainian документа на русский.

Все тяжёлые операции сериализованы: scan, index и translation не запускаются одновременно.
