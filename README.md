# OSINT Ukraine Analysis — local-first v0.9.0
...
Пока `osint-local serve` запущен, v0.9 сам поддерживает библиотеку в актуальном состоянии. Первый цикл начинается через несколько секунд после запуска, затем повторяется с интервалом выбранного performance-профиля:
...
Старые `config.json` без поля `performance` при переходе на v0.9 автоматически мигрируют в Economy.
