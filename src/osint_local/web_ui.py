...
<body>
<header class="topbar"><a class="brand" href="/">OSINT Local <span>v0.4</span></a><nav>
<a href="/search">Search</a><a href="/documents">Documents</a>
</nav></header>
<main>{body}</main>
...
def _action_panel(
    ...
    *,
    ...
    semantic_label = "Semantic ready" if semantic_ready else f"Semantic {embedding_count}/{chunk_count}"
    index_permanent_disabled = not semantic_available or chunk_count == 0
    index_disabled = running or index_permanent_disabled
    index_hint = ""
    if not semantic_available:
        index_hint = '<span class="action-hint">Install <code>.[search]</code> to enable semantic indexing.</span>'
    elif chunk_count == 0:
        index_hint = '<span class="action-hint">Scan documents first.</span>'
...
<form action="/actions/index" method="post" data-action-form><input type="hidden" name="csrf" value="{_e(csrf_token)}"><button class="secondary" type="submit"{index_disabled_attr}{index_permanent_attr}>Build index</button></form>
<a class="button ghost" href="/">Refresh</a>
</div>
<div class="action-meta"><span>{_e(semantic_label)}</span>{index_hint}</div>
